...
export function CancelBookingView({ bookingId, slug = "" }: { bookingId: string; slug?: string }) {
  ...
  if (canceled || booking.status === "CANCELLED") return <ManageOutcome title="Your appointment is canceled" description={booking.refundStatus === "REFUND_PENDING" ? "That time is open again. Your refund is queued for processing." : booking.refundStatus === "REFUNDED" ? `That time is open again and ${formatEuro(booking.refundedAmountCents / 100)} was refunded.` : booking.refundStatus === "REFUND_FAILED" ? "That time is open again, but the refund needs the studio’s attention." : "That time is open again. We hope to see you soon."} branding={branding} />;
...
function BookingCard({ booking, branding }: { booking: BookingSummary; branding: WorkspaceBranding | null }) {
  return <section className="confirmation-card"><div className="confirmation-brand"><BrandLogo branding={branding} /><div><strong>{booking.eventTitleSnapshot}</strong><span>{branding?.workspaceName || "Dvision Studio"}</span></div></div><dl><div><dt><Icon name="calendar" />Date and time</dt><dd>{bookingDate(booking)} at {bookingTime(booking)}<small>{booking.durationMinutes} minutes · {booking.inviteeTimeZone}</small></dd></div><div><dt><Icon name="location" />Where</dt><dd>{bookingLocation(booking)}<small>We’ll see you there</small></dd></div><div><dt><Icon name="calendar" />Confirmation</dt><dd>{calendarHeadline(booking)}<small>{calendarDelivery(booking)}</small></dd></div>{booking.priceCents > 0 && <div><dt><Icon name="sparkles" />Price</dt><dd>{formatEuro(booking.priceCents / 100)}<small>{booking.status === "CONFIRMED" ? "Paid at the shop" : booking.status.replaceAll("_", " ").toLowerCase()}</small></dd></div>}</dl></section>;
}
