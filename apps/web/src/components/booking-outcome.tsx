...
export function ConfirmationView({ slug, bookingId, payment, readCapability, cancelCapability, rescheduleCapability }: { slug: string; bookingId?: string; payment?: string; readCapability?: string; cancelCapability?: string; rescheduleCapability?: string }) {
  ...
  return <div className="public-page outcome-page"><BrandHeader branding={branding} /><main className="outcome-shell"><div className="success-mark" style={{ color: branding?.accentColor }}><Icon name="check" size={34} /></div><span className="outcome-eyebrow">Appointment confirmed</span><h1>You’re booked, {booking.inviteeName}.</h1><p>Your chair is reserved. Keep this page handy — you can reschedule or cancel here any time.</p><BookingCard booking={booking} branding={branding} /><div className="manage-row"><span>Need to change something?</span><Link href={`/manage/${booking.id}/reschedule?${slugQuery}`}>Reschedule</Link><i>·</i><Link href={`/manage/${booking.id}/cancel?${slugQuery}`}>Cancel</Link></div><small className="reference">Booking reference · {booking.reference ?? booking.id}</small></main><PublicFooter branding={branding} /></div>;
}
...
function ManageFrame({ title, description, branding, children }: { title: string; description: string; branding: WorkspaceBranding | null; children: ReactNode }) { return <div className="public-page manage-page"><BrandHeader branding={branding} /><main className="manage-shell"><span className="outcome-eyebrow">Manage appointment</span><h1>{title}</h1><p>{description}</p>{children}</main><PublicFooter branding={branding} /></div>; }
...
function ManageRetry({ bookingId, description, onRetry, branding = null }: { bookingId: string; description: string; onRetry: () => Promise<void>; branding?: WorkspaceBranding | null }) { return <div className="public-page manage-page"><BrandHeader branding={branding} /><main className="manage-shell"><span className="outcome-eyebrow">Secure link</span><h1>We couldn’t verify this appointment</h1><p role="alert">{description}</p><p>Try again, or request a fresh link below.</p><div className="auth-actions"><ActionButton variant="primary" onClick={() => void onRetry()}>Try again</ActionButton></div><BookingRecoveryForm bookingId={bookingId} /></main><PublicFooter branding={branding} /></div>; }
...
function ManageError({ title, description, branding = null }: { title: string; description: string; branding?: WorkspaceBranding | null }) { return <div className="public-page manage-page"><BrandHeader branding={branding} /><main className="manage-shell"><span className="outcome-eyebrow">Your appointment</span><h1>{title}</h1><p role="alert">{description}</p></main><PublicFooter branding={branding} /></div>; }
function ManageOutcome({ title, description, branding = null }: { title: string; description: string; branding?: WorkspaceBranding | null }) { return <div className="public-page outcome-page"><BrandHeader branding={branding} /><main className="outcome-shell"><div className="success-mark" style={{ color: branding?.accentColor }}><Icon name="check" size={34} /></div><span className="outcome-eyebrow">All set</span><h1>{title}</h1><p>{description}</p></main><PublicFooter branding={branding} /></div>; }
