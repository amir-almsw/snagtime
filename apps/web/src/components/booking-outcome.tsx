...
function calendarDelivery(booking: BookingSummary) {
  switch (booking.notificationStatus) {
    case "GOOGLE_UPDATE_ACCEPTED": return "Check your inbox for the calendar invitation.";
    case "LOCAL_NO_EMAIL": return "We’ve emailed your confirmation and manage link.";
    case "RETRY_PENDING": return "Your confirmation is on its way.";
    case "PENDING": return "Your confirmation is on its way.";
  }
}
function calendarHeadline(booking: BookingSummary) {
  switch (booking.notificationStatus) {
    case "GOOGLE_UPDATE_ACCEPTED": return "Invitation sent";
    case "LOCAL_NO_EMAIL": return "Confirmed by email";
...
function bookingLocation(booking: BookingSummary) {
  ...
  if (booking.locationType === "IN_PERSON") return booking.locationValue || "At the studio";
  return booking.locationValue || "Details to follow";
}
...
export function ConfirmationView({ slug, bookingId, payment, readCapability, cancelCapability, rescheduleCapability }: { slug: string; bookingId?: string; payment?: string; readCapability?: string; cancelCapability?: string; rescheduleCapability?: string }) {
  ...
  if (loading) return <ManageLoading label="Opening your appointment…" />;
  ...
  if (booking.status === "CANCELLED") return <ManageError title="This appointment is canceled" description="This appointment is no longer on the books. You’re welcome to book a new time whenever you’re ready." />;
  const slugQuery = `slug=${encodeURIComponent(slug)}`;
  return <div className="public-page outcome-page dvision"><BrandHeader branding={branding} /><main className="outcome-shell"><div className="success-mark" style={{ color: branding?.accentColor }}><Icon name="check" size={34} /></div><span className="outcome-eyebrow">Appointment confirmed</span><h1>You’re booked, {booking.inviteeName}.</h1><p>Your chair is reserved. Keep this page handy — you can reschedule or cancel here any time.</p><BookingCard booking={booking} branding={branding} /><div className="manage-row"><span>Need to change something?</span><Link href={`/manage/${booking.id}/reschedule?${slugQuery}`}>Reschedule</Link><i>·</i><Link href={`/manage/${booking.id}/cancel?${slugQuery}`}>Cancel</Link></div><small className="reference">Booking reference · {booking.reference ?? booking.id}</small></main><PublicFooter branding={branding} /></div>;
}
...
export function CancelBookingView({ bookingId, slug = "" }: { bookingId: string; slug?: string }) {
  ...
  if (loadingBooking) return <ManageLoading label="Loading your appointment…" branding={branding} />;
  if (!booking) return <ManageFrame title="We couldn’t open this appointment" description="Your secure link may have expired. Enter the email you booked with and we’ll send a fresh one." branding={branding}>{error && <div className="form-error" role="alert">{error}</div>}<BookingRecoveryForm bookingId={bookingId} onAccepted={() => setError("")} /></ManageFrame>;
  if (canceled || booking.status === "CANCELLED") return <ManageOutcome title="Your appointment is canceled" description={booking.refundStatus === "REFUND_PENDING" ? "That time is open again. Your refund is queued for processing." : booking.refundStatus === "REFUNDED" ? `That time is open again and ${(booking.refundedAmountCents / 100).toFixed(2)} ${booking.currency.toUpperCase()} was refunded.` : booking.refundStatus === "REFUND_FAILED" ? "That time is open again, but the refund needs the studio’s attention." : "That time is open again. We hope to see you soon."} branding={branding} />;
  return <ManageFrame title="Cancel this appointment" description="Check the details below before you cancel." branding={branding}><BookingManageSummary booking={booking} branding={branding} /><div className="manage-form">{booking.status === "PENDING_PAYMENT" && booking.priceCents > 0 && <div className="notice notice-info"><Icon name="sparkles" /><div><strong>Checkout was interrupted</strong><span>Your time is still on hold. Resume checkout or cancel the booking below.</span></div></div>}{booking.status === "PENDING_PAYMENT" && booking.priceCents > 0 && <ActionButton variant="primary" onClick={resumeCheckout} disabled={recovering}>{recovering ? "Opening checkout…" : "Resume checkout"}</ActionButton>}<Field label="Reason for canceling (optional)"><select value={reason} onChange={(item) => setReason(item.target.value)}><option value="">Choose a reason</option><option>Something came up</option><option>Booking another time</option><option>Booked by mistake</option><option>Other</option></select></Field>{error && <div className="form-error" role="alert">{error}</div>}<ActionButton variant="danger" onClick={cancel} disabled={loading}>{loading ? "Canceling…" : "Cancel booking"}</ActionButton></div></ManageFrame>;
}
...
export function RescheduleBookingView({ bookingId, slug = "" }: { bookingId: string; slug?: string }) {
  ...
  if (loadingBooking) return <ManageLoading label="Finding open times…" branding={branding} />;
  if (!booking) return <ManageFrame title="We couldn’t open this appointment" description="Your secure link may have expired. Enter the email you booked with and we’ll send a fresh one." branding={branding}>{error && <div className="form-error" role="alert">{error}</div>}<BookingRecoveryForm bookingId={bookingId} onAccepted={() => setError("")} /></ManageFrame>;
  if (done) return <ManageFrame title="Your appointment is moved" description={`We’ll see you ${bookingDate(booking)} at ${bookingTime(booking)} (${booking.inviteeTimeZone}).`} branding={branding}><BookingManageSummary booking={booking} branding={branding} />{error && <div className="form-error" role="alert">{error}</div>}{refreshedAfterSuccess && <ActionButton variant="secondary" onClick={() => { setDone(false); setRefreshedAfterSuccess(false); setError(""); }}>Choose another time</ActionButton>}</ManageFrame>;
  return <ManageFrame title="Choose a new time" description="Your current appointment stays reserved until you confirm the new one." branding={branding}><BookingManageSummary booking={booking} branding={branding} />{error && <div className="form-error" role="alert">{error}</div>}<div className="reschedule-picker"><div className="calendar-heading"><span>Available dates</span><div><button type="button" className="icon-button" aria-label="Previous available dates" disabled={dayOffset === 0} onClick={() => { const next = Math.max(0, dayOffset - 7); setDayOffset(next); setSelectedDate(days[next] ?? ""); setSelectedStart(""); }}><Icon name="arrow-left" /></button><button type="button" className="icon-button" aria-label="Next available dates" disabled={dayOffset + 7 >= days.length} onClick={() => { const next = Math.min(dayOffset + 7, Math.max(0, days.length - 1)); setDayOffset(next); setSelectedDate(days[next] ?? ""); setSelectedStart(""); }}><Icon name="arrow-right" /></button></div></div><div className="calendar-week">{visibleDays.map((key) => { const date = new Date(`${key}T12:00:00Z`); return <button type="button" className={activeDate === key ? "is-selected" : ""} aria-pressed={activeDate === key} onClick={() => { setSelectedDate(key); setSelectedStart(""); }} key={key}><span>{new Intl.DateTimeFormat("en-US", { weekday: "short", timeZone: booking.inviteeTimeZone }).format(date)}</span><strong>{new Intl.DateTimeFormat("en-US", { day: "numeric", timeZone: booking.inviteeTimeZone }).format(date)}</strong></button>; })}</div><div className="time-grid">{available.map((slot) => <button type="button" className={selectedStart === slot.start ? "is-selected" : ""} aria-pressed={selectedStart === slot.start} onClick={() => setSelectedStart(slot.start)} key={slot.start}>{new Intl.DateTimeFormat("en-US", { hour: "numeric", minute: "2-digit", timeZone: booking.inviteeTimeZone }).format(new Date(slot.start))}{selectedStart === slot.start && <Icon name="check" size={15} />}</button>)}</div>{slots.length === 0 && <div className="empty-state"><p>No other times are open right now. Contact the studio and we’ll find one for you.</p></div>}<ActionButton variant="primary" className="flow-next" disabled={loading || !selectedStart} onClick={reschedule}>{loading ? "Rescheduling…" : "Confirm new time"} <Icon name="arrow-right" /></ActionButton></div></ManageFrame>;
}

function BookingCard({ booking, branding }: { booking: BookingSummary; branding: WorkspaceBranding | null }) {
  return <section className="confirmation-card"><div className="confirmation-brand"><BrandLogo branding={branding} /><div><strong>{booking.eventTitleSnapshot}</strong><span>{branding?.workspaceName || "Dvision Studio"}</span></div></div><dl><div><dt><Icon name="calendar" />Date and time</dt><dd>{bookingDate(booking)} at {bookingTime(booking)}<small>{booking.durationMinutes} minutes · {booking.inviteeTimeZone}</small></dd></div><div><dt><Icon name="location" />Where</dt><dd>{bookingLocation(booking)}<small>We’ll see you there</small></dd></div><div><dt><Icon name="calendar" />Confirmation</dt><dd>{calendarHeadline(booking)}<small>{calendarDelivery(booking)}</small></dd></div>{booking.priceCents > 0 && <div><dt><Icon name="sparkles" />Price</dt><dd>${(booking.priceCents / 100).toFixed(2)} {booking.currency.toUpperCase()}<small>{booking.status === "CONFIRMED" ? "Paid at the shop" : booking.status.replaceAll("_", " ").toLowerCase()}</small></dd></div>}</dl></section>;
}
function PaymentPending({ booking, branding, payment, slug }: { booking: BookingSummary; branding: WorkspaceBranding | null; payment?: string; slug: string }) {
  ...
  const slugQuery = `slug=${encodeURIComponent(slug)}`;
  return <div className="public-page outcome-page dvision"><BrandHeader branding={branding} /><main className="outcome-shell"><span className="outcome-eyebrow">Payment pending</span><h1>{payment === "success" ? "We’re confirming your payment" : "Your booking is not confirmed yet"}</h1><p>{payment === "success" ? "Checkout is complete. Refresh in a moment while we confirm the payment." : "Checkout was canceled or interrupted. You can pick up where you left off."}</p><BookingCard booking={booking} branding={branding} />{error && <div className="form-error" role="alert">{error}</div>}<div className="manage-row">{payment === "success" ? <ActionButton variant="primary" onClick={() => window.location.reload()}>Refresh payment status</ActionButton> : <ActionButton variant="primary" onClick={resume} disabled={working}>{working ? "Opening checkout…" : "Resume checkout"}</ActionButton>}<Link href={`/manage/${booking.id}/cancel?${slugQuery}`}>Cancel booking</Link></div><small className="reference">Booking reference · {booking.reference ?? booking.id}</small></main><PublicFooter branding={branding} /></div>;
}
...
function ManageFrame({ title, description, branding, children }: { title: string; description: string; branding: WorkspaceBranding | null; children: ReactNode }) { return <div className="public-page manage-page dvision"><BrandHeader branding={branding} /><main className="manage-shell"><span className="outcome-eyebrow">Manage appointment</span><h1>{title}</h1><p>{description}</p>{children}</main><PublicFooter branding={branding} /></div>; }
function ManageLoading({ label, branding = null }: { label: string; branding?: WorkspaceBranding | null }) { return <div className="public-page manage-page dvision"><BrandHeader branding={branding} /><main className="manage-shell" role="status"><span className="spinner" /><p>{label}</p></main><PublicFooter branding={branding} /></div>; }
function ManageRetry({ bookingId, description, onRetry, branding = null }: { bookingId: string; description: string; onRetry: () => Promise<void>; branding?: WorkspaceBranding | null }) { return <div className="public-page manage-page dvision"><BrandHeader branding={branding} /><main className="manage-shell"><span className="outcome-eyebrow">Secure link</span><h1>We couldn’t verify this appointment</h1><p role="alert">{description}</p><p>Try again, or request a fresh link below.</p><div className="auth-actions"><ActionButton variant="primary" onClick={() => void onRetry()}>Try again</ActionButton></div><BookingRecoveryForm bookingId={bookingId} /></main><PublicFooter branding={branding} /></div>; }
function BookingRecoveryForm({ bookingId, onAccepted }: { bookingId: string; onAccepted?: () => void }) { const [email, setEmail] = useState(""); const [working, setWorking] = useState(false); const [accepted, setAccepted] = useState(false); const [error, setError] = useState(""); const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setWorking(true); setError(""); try { await frontendApi.requestBookingManageLink(bookingId, email.trim()); setAccepted(true); onAccepted?.(); } catch (reason) { setError(reason instanceof Error ? reason.message : "The request could not be accepted."); } finally { setWorking(false); } }; if (accepted) return <div className="notice notice-info" role="status"><Icon name="check" /><div><strong>Request received</strong><span>If that email matches this appointment, a fresh link is on its way. For your privacy we don’t confirm either way here.</span></div></div>; return <form className="manage-recovery-form" onSubmit={submit}><Field label="Booking email" required hint="For your privacy, we don’t reveal whether an appointment matches."><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" required /></Field>{error && <div className="form-error" role="alert">{error}</div>}<ActionButton variant="secondary" type="submit" disabled={working || !email.includes("@")} >{working ? "Sending…" : "Email me a new link"}</ActionButton></form>; }
function ManageError({ title, description, branding = null }: { title: string; description: string; branding?: WorkspaceBranding | null }) { return <div className="public-page manage-page dvision"><BrandHeader branding={branding} /><main className="manage-shell"><span className="outcome-eyebrow">Your appointment</span><h1>{title}</h1><p role="alert">{description}</p></main><PublicFooter branding={branding} /></div>; }
function ManageOutcome({ title, description, branding = null }: { title: string; description: string; branding?: WorkspaceBranding | null }) { return <div className="public-page outcome-page dvision"><BrandHeader branding={branding} /><main className="outcome-shell"><div className="success-mark" style={{ color: branding?.accentColor }}><Icon name="check" size={34} /></div><span className="outcome-eyebrow">All set</span><h1>{title}</h1><p>{description}</p></main><PublicFooter branding={branding} /></div>; }
function BrandLogo({ branding }: { branding: WorkspaceBranding | null }) { const initial = branding?.workspaceName.charAt(0).toUpperCase() || "D"; return <span className="public-logo" style={{ background: branding?.accentColor, color: foregroundForBackground(branding?.accentColor) }}>{branding?.logoUrl ? <span role="img" aria-label={`${branding.workspaceName} logo`} style={{ display: "block", width: "100%", height: "100%", borderRadius: "inherit", background: `#fff center / contain no-repeat url(${JSON.stringify(branding.logoUrl)})` }} /> : initial}</span>; }
function BrandHeader({ branding }: { branding: WorkspaceBranding | null }) {
  // Only rendered where a provider supplies a target -- the manage pages. The booking flow has none on
  // purpose: someone mid-way through picking a time should not be handed an exit.
  const back = useBackTarget();
  return <header className="public-header"><div className="booking-brand"><BrandLogo branding={branding} /><div><strong>{branding?.workspaceName || "Dvision Studio"}</strong><span>{branding?.description || "By appointment"}</span></div></div>{back && <Link className="header-back-link" href={back.href}><Icon name="arrow-left" size={14} />{back.label}</Link>}</header>;
}
function PublicFooter({ branding }: { branding: WorkspaceBranding | null }) { return <footer className="public-footer"><span>{branding?.footerText || "Dvision Studio"}</span><span>Your details stay private</span></footer>; }
