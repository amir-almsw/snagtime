...
import { dayKey } from "./bookings-calendar-state";
...
// The barber's last-used layout, kept per browser like the appearance switch. Calendar is the default.
const VIEW_STORAGE_KEY = "dvision:bookings-view";
type BookingsLayout = "calendar" | "list";
// Read once as the initial state. On the server (no window) and in private browsing the read throws
// and the default wins; the first paint is the loading state either way, so hydration has nothing to diff.
function storedLayout(): BookingsLayout { try { return window.localStorage.getItem(VIEW_STORAGE_KEY) === "list" ? "list" : "calendar"; } catch { return "calendar"; } }
...
export function BookingsView() {
  ...
  const [view, setView] = useState<BookingsLayout>(storedLayout);
  ...
  const load = useCallback(() => { frontendApi.getAccount().then(async (account) => { const [bookingItems, eventItems] = await Promise.all([frontendApi.listBookings(account.workspace.timeZone), frontendApi.listEventTypes()]); const events = new Map(eventItems.map((event) => [event.id, event])); const resolved = bookingItems.map((booking) => { const event = events.get(booking.eventTypeId); return { ...booking, ...(event ? { eventSlug: event.slug } : {}) }; }); setWorkspaceTimeZone(account.workspace.timeZone); setBookings(resolved); setServices(eventItems.filter((event) => event.status === "published")); const requested = new URLSearchParams(window.location.search).get("selected"); const requestedBooking = requested ? resolved.find((booking) => booking.id === requested) ?? null : null; if (requested) setSelected(requestedBooking); setAnchorDay(dayKey(requestedBooking?.startsAt ?? new Date(), account.workspace.timeZone)); }).catch((reason) => setError(reason instanceof Error ? reason.message : "Could not load bookings.")).finally(() => setLoading(false)); }, []);
  useEffect(() => { void load(); }, [load]);
  const chooseView = (next: BookingsLayout) => { setView(next); try { window.localStorage.setItem(VIEW_STORAGE_KEY, next); } catch { /* the choice lasts for this page only */ } };
  const openBooking = (booking: Booking, trigger: HTMLButtonElement) => { returnFocusRef.current = trigger; const url = new URL(window.location.href); url.searchParams.set("selected", booking.id); window.history.replaceState(null, "", `${url.pathname}${url.search}`); setSelected(booking); };
  ...
  return <div className="page-stack">
    <PageHeader title="Bookings" description={`Studio dates and times are shown in ${workspaceTimeZone}.${view === "calendar" ? " Canceled bookings appear in the list only." : ""}`} actions={canManage && <button ref={addRef} type="button" className="button button-primary" onClick={() => setAdding(true)} aria-haspopup="dialog"><Icon name="plus" size={16} />Add booking</button>} />
    {error && <div className="toast toast-error" role="alert"><span><Icon name="x" /></span>{error}</div>}
    <div className="toolbar bookings-toolbar"><div className="search-field"><Icon name="search" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by name, email, or event" aria-label="Search bookings" /></div><select value={status} onChange={(e) => setStatus(e.target.value as typeof status)} aria-label="Filter booking status"><option value="all">All server statuses</option><option value="confirmed">Confirmed</option><option value="pending">Pending payment</option><option value="canceled">Canceled</option></select><div className="segmented" aria-label="Bookings layout">{(["calendar", "list"] as const).map((item) => <button type="button" key={item} className={view === item ? "is-active" : ""} aria-pressed={view === item} onClick={() => chooseView(item)}>{item === "calendar" ? "Calendar" : "List"}</button>)}</div></div>
    {view === "calendar" && <BookingsCalendar key={anchorDay} bookings={filtered} timeZone={workspaceTimeZone} anchorDay={anchorDay || dayKey(new Date(), workspaceTimeZone)} onOpen={openBooking} />}
    {view === "list" && <section className="panel bookings-panel">
      <div className="booking-table-head"><span>Client</span><span>Event</span><span>Date & time</span><span>Status</span><span /></div>
      <div className="booking-table">{filtered.map((booking) => <button type="button" className="booking-table-row" key={booking.id} onClick={(event) => openBooking(booking, event.currentTarget)} aria-haspopup="dialog"><span className="invitee-cell"><Avatar name={booking.invitee} /><span><strong>{booking.invitee}</strong><small>{booking.email}</small></span></span><span><strong>{booking.eventTitle}</strong><small>{booking.duration} min</small></span><span><strong>{booking.dateLabel}</strong><small>{booking.timeLabel}</small></span><span><Badge tone={tones[booking.status]} dot>{booking.status}</Badge></span><span><Icon name="arrow-right" /></span></button>)}</div>
