...
import { PRICE_CURRENCY, PRICE_SYMBOL, priceFromInput, priceInputValue } from "./price-input";
import { Icon } from "./icons";
import { ActionButton, Badge, Field, Toggle } from "./ui";
import { useWorkspaceAccess } from "./workspace-access";

const defaultEvent: EventType = {
  id: "new", title: "", slug: "", description: "", color: "#2563eb", status: "draft", location: "In person",
  locationType: "IN_PERSON", locationValue: null,
  durations: [{ minutes: 30, label: "30 min", isDefault: true, currency: PRICE_CURRENCY }], questions: [],
...
export function EventTypeEditor({ eventId, mode = "edit" }: { eventId?: string; mode?: "edit" | "create" }) {
  ...
  // A duration is paid when it carries a price at all; 0 is a paid row the barber is still typing.
  const isPaid = event.durations.some((duration) => duration.price !== undefined);
  const currency = PRICE_CURRENCY;
  const publicUrl = `/book/${event.slug || "your-link"}`;
  const locationComplete = Boolean(event.locationValue?.trim());
  ...
  useEffect(() => {
    ...
    frontendApi.getEventType(eventId).then((item) => {
      ...
      setEvent({ ...item, locationType: "IN_PERSON", location: item.locationValue || "In person" });
      setPersistedSlug(item.status === "published" && item.slug ? item.slug : null);
      setLoaded(true);
    }).catch((reason) => { if (active) setSaveError(reason instanceof Error ? reason.message : "Could not load this service."); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  ...
  const canPublish = Boolean(event.title.trim() && event.slug.trim() && event.durations.length > 0 && locationComplete);
  ...
  async function save(publish = false) {
    ...
    try {
      ...
      setSaveError(error instanceof Error ? error.message : "Could not save this service.");
    } finally { setSaving(false); }
  }

  async function remove() {
    if (mode !== "edit" || !eventId || deleting) return;
    if (!window.confirm(`Delete “${event.title || "this service"}”? Existing bookings remain in history, but its public booking page will stop accepting bookings.`)) return;
    setDeleting(true); setSaveError("");
    try { await frontendApi.deleteEventType(eventId); router.replace("/event-types"); }
    catch (reason) { setSaveError(reason instanceof Error ? reason.message : "Could not delete this service."); setDeleting(false); }
  }

  if (!canManage) return <div className="editor-page"><section className="empty-state" role="alert"><span className="empty-icon"><Icon name="x" /></span><h1>Studio access required</h1><p>Your workspace role cannot create or change services.</p><Link href="/dashboard" className="button button-secondary">Back to dashboard</Link></section></div>;
  if (mode === "edit" && loading) return <div className="editor-page"><div className="sync-note" role="status"><span className="spinner" />Loading service…</div></div>;
  if (mode === "edit" && !loaded) return <div className="editor-page"><section className="empty-state" role="alert"><span className="empty-icon"><Icon name="x" /></span><h1>Service unavailable</h1><p>{saveError || "This service could not be loaded."}</p><ActionButton variant="primary" onClick={() => { setLoading(true); setSaveError(""); setLoadAttempt((attempt) => attempt + 1); }}>Retry</ActionButton><Link href="/event-types" className="button button-secondary">Back to services</Link></section></div>;

  return <div className="editor-page">
    <header className="editor-topbar">
      <div className="editor-title"><Link href="/event-types" className="icon-button" aria-label="Back to services"><Icon name="arrow-left" /></Link><span className="service-color-dot" aria-hidden="true" style={{ background: event.color }} /><div><span>{mode === "create" ? "New service" : "Editing service"}</span><strong>{event.title || "Untitled service"}</strong></div><Badge tone={event.status === "published" ? "success" : "neutral"} dot>{event.status}</Badge></div>
      <div className="editor-actions">{mode === "edit" && <ActionButton onClick={() => void remove()} variant="danger" disabled={deleting || saving}>{deleting ? "Deleting…" : "Delete"}</ActionButton>}{persistedSlug ? <Link href={`/book/${persistedSlug}`} className="button button-secondary"><Icon name="external" size={16} />Preview published page</Link> : <button type="button" className="button button-secondary" disabled title="Save and publish this service before previewing"><Icon name="external" size={16} />Preview unavailable</button>}{event.status !== "published" && <ActionButton onClick={() => save(false)} variant="secondary" disabled={saving || deleting}>{saving ? "Saving…" : "Save draft"}</ActionButton>}<ActionButton onClick={() => save(true)} variant="primary" disabled={!canPublish || saving || deleting}>{saving ? "Saving…" : event.status === "published" ? "Save changes" : "Publish service"}</ActionButton></div>
    </header>
    ...
    <div className="editor-layout">
      ...
      <section className="editor-content">
        ...
        {activeTab === "Basics" && <div className="editor-sections">
          <section className="form-card"><div className="form-card-title"><span className="number-chip">1</span><div><h2>Service details</h2><p>Give clients a clear reason to book.</p></div></div><div className="form-grid">
            <Field label="Service name" required><input value={event.title} onChange={(e) => update("title", e.target.value)} placeholder="e.g. Skin Fade" /></Field>
            ...
            <Field label="Description"><textarea rows={4} value={event.description} onChange={(e) => update("description", e.target.value)} placeholder="Describe what this service includes and anything clients should know." /></Field>
            <Field label="Service color"><div className="color-options" role="radiogroup" aria-label="Service color">{["#2563eb", "#16a394", "#3978d4", "#ed7a5f", "#e2a93b", "#8f5db7"].map((color) => <button type="button" role="radio" aria-checked={event.color === color} key={color} className={event.color === color ? "is-selected" : ""} style={{ background: color, color: foregroundForBackground(color) }} onClick={() => update("color", color)} aria-label={`Use color ${color}`}><Icon name="check" size={14} /></button>)}</div></Field>
          </div></section>
          <section className="form-card"><div className="form-card-title"><span className="number-chip">2</span><div><h2>Duration options</h2><p>Clients pick one duration before they see your open times.</p></div></div><div className="duration-list" role="radiogroup" aria-label="Default duration">
            {event.durations.map((duration, index) => <div className="duration-row" key={duration.id ?? `new-${index}`}>
              ...
              <select aria-label={`Payment type for ${duration.label}`} value={duration.price !== undefined ? "paid" : "free"} onChange={(e) => updateDuration(index, { price: e.target.value === "paid" ? 25 : undefined, currency })}><option value="free">Free</option><option value="paid">Paid</option></select>
              {duration.price !== undefined ? <div className="money-input"><span>{PRICE_SYMBOL}</span><input aria-label={`Price for ${duration.label}`} type="number" min="0" step="0.01" inputMode="decimal" value={priceInputValue(duration.price)} onChange={(e) => updateDuration(index, { price: priceFromInput(e.target.value) })} /></div> : <span className="muted">No payment</span>}
          ...
          </div>{isPaid && <div className="inline-fields"><div className="notice notice-info"><Icon name="sparkles" /><div><strong>Price is shown in euro, not charged</strong><span>Clients see this price when booking and pay at the shop. No online payment is taken.</span></div></div></div>}</section>
          {/* Every service happens at the shop: the type is fixed and the address is what has to be filled in. */}
          <section className="form-card"><div className="form-card-title"><span className="number-chip">3</span><div><h2>Location</h2><p>Say where clients should come and add anything they need to know.</p></div></div><Field label="Location type"><select defaultValue="IN_PERSON"><option value="IN_PERSON">In person</option></select></Field><Field label="Studio address" required hint="These details are saved with the booking and shown to your client."><input value={event.locationValue ?? ""} onChange={(item) => { update("locationValue", item.target.value); update("location", item.target.value || "In person"); }} required /></Field></section>
        </div>}
