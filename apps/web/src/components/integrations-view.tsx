...
export function IntegrationsView() {
  ...
  return <div className="page-stack">
    ...
    <div className="integration-groups"><section><div className="group-title"><h2>Calendar</h2><span>1 integration</span></div><div className="integration-grid">
      <article className="integration-card"><div className="integration-mark mark-google"><GoogleCalendarLogo /></div><div className="integration-copy"><div><h3>Google Calendar</h3>{google && <Badge tone={google.disconnectPending || reconnectRequired || googleUnavailable || localFallback ? "warning" : google.connected ? "success" : google.configured ? "neutral" : "warning"} dot>{google.disconnectPending ? "Disconnect pending" : reconnectRequired ? "Reconnect required" : googleUnavailable ? "Unavailable" : localFallback ? "Disconnected" : google.connected ? "Connected" : google.configured ? "Available" : "Not configured"}</Badge>}</div><p>{google?.connected && google.provider === "google" ? "Appointments are mirrored to Google Calendar and its busy time is added to the studio schedule." : reconnectRequired ? "Reconnect Google Calendar to resume mirroring appointments. Clients keep booking from the studio schedule." : googleUnavailable ? "Google Calendar could not be reached. Clients keep booking from the studio schedule; reconnect to resume mirroring." : localFallback ? "Connect Google Calendar to mirror appointments to your calendar. Clients already book from the studio schedule." : "Connect Google Calendar to mirror appointments to your calendar."}</p>{google && <span className="integration-detail">{google.connected ? `${google.calendarId} calendar · ${google.scopeHealth === "complete" ? "Full calendar access" : "Access needs attention"}` : google.missingScopes.length ? `${google.missingScopes.length} permission${google.missingScopes.length === 1 ? "" : "s"} needed` : "No calendar connected"}</span>}</div>{canManage && (google?.disconnectPending ? <ActionButton variant="secondary" disabled>Disconnect pending</ActionButton> : google?.connected ? google.disconnectSupported ? <ActionButton variant="secondary" onClick={disconnect} disabled={working}>{working ? "Disconnecting…" : "Disconnect"}</ActionButton> : <Badge tone="neutral">Managed externally</Badge> : <ActionButton variant="primary" onClick={connect} disabled={working || !google?.configured}>{working ? "Opening…" : reconnectRequired ? "Reconnect" : "Connect"}</ActionButton>)}</article>
    </div></section><section><div className="group-title"><h2>Payments</h2><span>Test checkout</span></div><div className="integration-grid"><article className="integration-card"><div className="integration-mark mark-stripe"><StripeLogo /></div><div className="integration-copy"><div><h3>Stripe</h3>{stripe && <Badge tone={stripe.configured ? "success" : "warning"} dot>{stripe.configured ? "Test mode ready" : "Setup needed"}</Badge>}</div><p>{stripe?.configured ? "Test checkout is ready for paid services." : "Add your Stripe test credentials before publishing paid services."}</p>{stripe && <span className="integration-detail">{stripe.mode === "test" ? "Test payments only" : stripe.mode}</span>}</div></article></div></section>
