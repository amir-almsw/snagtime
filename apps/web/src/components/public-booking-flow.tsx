...
const fallbackTimeZones = ["Europe/Amsterdam", "UTC", "Europe/London", "Europe/Berlin", "Europe/Paris"];
...
export function PublicBookingFlow({ slug }: { slug: string }) {
  ...
  async function confirm() {
    ...
    try {
      ...
      // One live appointment per client: acknowledge the block, then hand them their existing booking.
      if (reason instanceof SnagTimeApiError && reason.code === "ACTIVE_BOOKING_EXISTS") {
        window.alert(`${reason.message}\n\nOpen your existing appointment and use Reschedule to move it.`);
        if (reason.bookingId) router.push(`/book/${slug}/confirmation?booking=${encodeURIComponent(reason.bookingId)}`);
        else setError(reason.message);
        return;
      }
      const message = reason instanceof Error ? reason.message : "That time has just been taken. Please choose another.";
  ...
  if (!event || !duration) return <div className="public-page"><header className="public-header"><BrandMark /></header><main className="outcome-shell"><h1>This service isn’t bookable right now</h1><p>{error || "No times are set up for this service yet. Contact the studio and we’ll take care of you."}</p></main></div>;
  const branding = event.branding;

  return <div className="public-page">
    <header className="public-header"><div className="public-workspace-brand"><PublicBrandLogo branding={branding} /><strong>{branding?.workspaceName || "Dvision Studio"}</strong></div><div><Icon name="globe" /><select value={timezone} onChange={(item) => { setLoadingSlots(true); resetScheduleProgress(); setTimezone(item.target.value); }} aria-label="Booking timezone">{timeZones.map((zone) => <option value={zone} key={zone}>{timeZoneLabel(zone)}</option>)}</select></div></header>
    <main className="booking-shell">
      <aside className="booking-info"><span className="host-label">At {branding?.workspaceName || "Dvision Studio"}</span><h1>{event.title}</h1><p>{event.description}</p>{branding?.description && <p className="workspace-description">{branding.description}</p>}<div className="public-meta"><span><Icon name="clock" />{duration.label}</span><span><Icon name="video" />{event.location}</span><span><Icon name="globe" />{timeZoneLabel(timezone)}</span>{paid && <span><Icon name="sparkles" />${duration.price?.toFixed(2)} {currencyLabel(duration.currency)} · pay at the shop</span>}</div><div className="booking-safe-note"><Icon name="check" /><span>Your chair is held the moment you book. No account, no deposit.</span></div></aside>
      <section className="booking-flow" aria-busy={step === "schedule" && loadingSlots} aria-label="Booking steps">
        ...
        {availabilityNotice && <div className="notice notice-warning" role="status"><Icon name="calendar" /><div><strong>That time just went</strong><span>{availabilityNotice}</span></div></div>}
        {step === "schedule" && <div className="flow-panel" id="booking-step-panel">
          <div className="flow-heading"><h2 ref={stepHeadingRef} tabIndex={-1}>Choose a duration</h2><p>How long would you like in the chair?</p></div>
          ...
          {loadingSlots ? <div className="sync-note" role="status"><span className="spinner" />Finding open times…</div> : days.length ? <div className="booking-calendar"><div className="calendar-week">{visibleDays.map((day) => <button type="button" className={selectedDate === day.key ? "is-selected" : ""} aria-pressed={selectedDate === day.key} onClick={() => { setSelectedDate(day.key); resetScheduleProgress(); }} key={day.key}><span>{day.weekday}</span><strong>{day.day}</strong><i>{day.month}</i></button>)}</div><div className="time-grid-scroll" role="region" aria-label={`Available times for ${selectedDay?.label ?? "selected date"}`} tabIndex={0}><div className="time-grid">{daySlots.map(({ slot, time }) => <button type="button" className={selectedStart === slot.start ? "is-selected" : ""} aria-pressed={selectedStart === slot.start} onClick={() => { setSelectedStart(slot.start); setFurthestStep("schedule"); setError(""); setAvailabilityNotice(""); }} key={slot.start}>{time}{selectedStart === slot.start && <Icon name="check" size={15} />}</button>)}</div></div></div> : <div className="empty-state"><span className="empty-icon"><Icon name="calendar" /></span><h3>No open times</h3><p>Nothing is free in this window. Try another timezone, or contact the studio.</p></div>}
