...
export function WorkspaceOnboarding() {
  ...
  return <div className="auth-page onboarding-page"><main className="auth-card onboarding-card"><BrandMark /><div className="onboarding-success"><Icon name="check" size={24} /></div><div><span className="outcome-eyebrow">Your workspace</span><h1>Everything looks ready</h1><p>Review the details below, then open your scheduling dashboard.</p></div><dl className="onboarding-summary"><div><dt>Workspace</dt><dd>{account.workspace.name}</dd></div><div><dt>Timezone</dt><dd>{account.workspace.timeZone}</dd></div><div><dt>Your role</dt><dd><Badge tone="brand">{account.workspace.role.toLowerCase()}</Badge></dd></div><div><dt>Barber</dt><dd>{account.user.name}<small>{account.user.email}</small></dd></div></dl><div className="onboarding-next"><strong>Next steps</strong><span>Set your availability, create a booking link, and connect the tools you use.</span></div>{error && <div className="form-error" role="alert" aria-live="assertive">{error}</div>}<ActionButton variant="primary" onClick={complete} disabled={completing}>{completing ? "Opening…" : "Open dashboard"}<Icon name="arrow-right" /></ActionButton></main></div>;
}
