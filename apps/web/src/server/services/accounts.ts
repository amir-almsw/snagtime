...
export async function registerAccount(input: RegistrationInput, observeWork: (phase: RegistrationWorkPhase) => void = observeNoRegistrationWork, now = new Date()) {
  ...
  await db.$transaction(async (tx) => {
    ...
    await tx.$executeRaw`INSERT INTO "WorkspaceBranding" ("id","workspaceId","userId","workspaceName","accentColor") SELECT ${brandingId},${workspaceId},${userId},${input.workspaceName},'#C11427' WHERE ${created}=1`; observeWork("BRANDING_INSERT");
