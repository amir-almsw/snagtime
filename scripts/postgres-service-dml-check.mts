...
try{
  ...
  // Under the app role and RLS the public slot list must hide the booked time from the database alone (the proof provider reports no busy time).
  const publicSlots=await bookings.listPublicSlots(`service-free-${suffix}`,DateTime.fromJSDate(start).minus({hours:1}).toJSDate(),DateTime.fromJSDate(start).plus({hours:1}).toJSDate(),"UTC",calendar,durationId);
  if(!publicSlots.length||publicSlots.some(slot=>new Date(slot.start).getTime()===start.getTime()))throw new Error(`Public slot list did not hide the booked time from the database ${JSON.stringify({slots:publicSlots.length})}`);
  ...
  // The dashboard's own booking form. Two things are new here and neither is observable under SQLite:
  // createBooking runs with a workspace context already in the store before it enters its public one,
  // and the studio is exempt from the one-live-appointment rule, so this books the same client twice.
  const hostStart=DateTime.fromJSDate(start).plus({days:5}).toJSDate();
  const hostBooking=await dbContext.run(base,()=>bookings.createBooking(`service-free-${suffix}`,{startAt:hostStart.toISOString(),inviteeName:"Studio added",inviteeEmail:`free-${suffix}@example.invalid`,inviteeTimeZone:"UTC",durationId},`host-${suffix}`,calendar,payments,{allowSecondActiveBooking:true}));
  const [hostStored,hostActive,hostOccupancy]=await Promise.all([proof.booking.findUniqueOrThrow({where:{id:hostBooking.booking.id}}),proof.booking.count({where:{workspaceId,inviteeEmail:`free-${suffix}@example.invalid`,status:{in:["CONFIRMED","PENDING_PAYMENT"]}}}),proof.bookingOccupancy.count({where:{bookingId:hostBooking.booking.id}})]);
  if(hostStored.status!=="CONFIRMED"||hostActive!==2||hostOccupancy===0)throw new Error(`Organizer-created booking did not reach PostgreSQL under the public booking policies ${JSON.stringify({status:hostStored.status,hostActive,hostOccupancy})}`);
